...
public class MySQLTableCreation {
		...
		public static void main(String[] args) {
			try {
				...
				//Create four tables: items, categories, users, history
...
/*
*******************************************************************************
 * SQL insert items table
 * CREATE TABLE items (item_id VARCHAR(255) NOT NULL, name VARCHAR(255), 
 *					city VARCHAR(255), state VARCHAR(255), country VARCHAR(255),
 *					zipcode VARCHAR(255), rating FLOAT, address VARCHAR(255), 
 *					latitude FLOAT, longitude FLOAT, description VARCHAR(255), 
 *					url VARCHAR(255),PRIMARY KEY ( item_id ));
 *
 * SQL insert users table
 * CREATE TABLE users (user_id VARCHAR(255) NOT NULL, password VARCHAR(255) NOT NULL, 
 *					   first_name VARCHAR(255), last_name VARCHAR(255), PRIMARY KEY ( user_id ));
 *
 * SQL insert categories table
 * CREATE TABLE categories (item_id VARCHAR(255) NOT NULL, category VARCHAR(255), 
 * 							PRIMARY KEY ( item_id, category), FOREIGN KEY (item_id) REFERENCES items(item_id));
 * 
 * SQL insert history table 
 * CREATE TABLE history (history_id bigint(20) unsigned NOT NULL AUTO_INCREMENT, 
 *						user_id VARCHAR(255) NOT NULL, item_id VARCHAR(255) NOT NULL, 
 *						last_favor_time timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,PRIMARY KEY (history_id),
 *						UNIQUE KEY `user_item_id` (`user_id`,`item_id`), 
 *						FOREIGN KEY (item_id) REFERENCES items(item_id),
